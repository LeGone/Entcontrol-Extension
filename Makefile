...
ifeq "$(USEMETA)" "true"
	...
	ifeq "$(ENGINE)" "csgo"
		LINK_HL2 += $(HL2LIB)/interfaces_i486.a $(LIB_PREFIX)vstdlib$(LIB_SUFFIX) $(LIB_PREFIX)tier0$(LIB_SUFFIX)
	else
...
all: check
	mkdir -p $(BIN_DIR)/sdk
	mkdir -p $(BIN_DIR)/Entity
	mkdir -p $(BIN_DIR)/Nav
	mkdir -p $(BIN_DIR)/HLLib
	mkdir -p $(BIN_DIR)/HLLib/HLLib
	...
	if [ "$(USEMETA)" = "true" ]; then \
		...
		ln -sf $(HL2LIB)/$(LIB_PREFIX)tier1_srv$(LIB_SUFFIX); \
	fi
